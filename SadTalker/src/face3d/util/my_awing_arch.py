...
class AddCoordsTh(nn.Module):
    ...
    def forward(self, input_tensor, heatmap=None):
        ...
        xx_range = torch.arange(
            self.x_dim, dtype=torch.int32, device=input_tensor.device).unsqueeze(0)
        ...
        yy_range = torch.arange(
            self.y_dim, dtype=torch.int32, device=input_tensor.device).unsqueeze(0)
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes):
        ...
        self.conv2 = conv3x3(int(out_planes / 2),
                             int(out_planes / 4), padding=1, dilation=1)
        self.bn3 = nn.BatchNorm2d(int(out_planes / 4))
        self.conv3 = conv3x3(int(out_planes / 4),
                             int(out_planes / 4), padding=1, dilation=1)
...
class FAN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_modules):
            hg, boundary_channel = self._modules['m' +
